...
def ddp_setup(rank, world_size):
    """
    Set up DistributedDataParallel (DDP) environment for Linux.

    Args:
        rank (int): Unique ID of the current process.
        world_size (int): Total number of processes (e.g., GPUs).
    """
    # Set master address and port only if not already defined
    os.environ.setdefault("MASTER_ADDR", "localhost")
    os.environ.setdefault("MASTER_PORT", "12345")

    # Initialize the process group using NCCL backend (optimized for GPU on Linux)
    init_process_group(backend="nccl", rank=rank, world_size=world_size)

    # Assign the current process to a specific GPU
    torch.cuda.set_device(rank)
